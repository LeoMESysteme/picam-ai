...
def _digit_blobs(mask, min_height, max_height, close_kernel):
    ...
    closed = cv2.morphologyEx(mask, cv2.MORPH_CLOSE, np.ones((close_kernel, close_kernel), np.uint8))
...
def fit_ocr_box(rectified_crop, layout: DisplayLayout | None = None):
    ...
    Blende aus, ohne sie explizit zu kennen. Bekannte Grenze: bei sehr
    schmalen Layouts (wenige, breite Ziffernzellen) kann der Schliess-Kernel
    einzelne Ziffern uneinheitlich in Ober-/Unterhaelfte zerfallen lassen,
    wodurch das groesste Cluster nur einen Teil der Zeile trifft - unvalidiert
    ohne weitere reale Beispiele dieser Layoutklasse. `None` bei zu wenig Evidenz oder
    ...
    close_kernel = max(3, round(height * 0.08))

    best_blobs = None
    for mask in _threshold_variants(gray):
        blobs = _digit_blobs(mask, min_height, max_height, close_kernel)
        if len(blobs) >= 2 and (best_blobs is None or len(blobs) > len(best_blobs)):
            best_blobs = blobs
    if best_blobs is None:
        return None

    # Groesstes Cluster nach Gesamtflaeche behalten - schliesst Einheitentext
    # und Blende aus, ohne sie zu kennen. Bekannte Grenze: eine echte
    # Nebenanzeige im selben Ausschnitt (Konzept.md §7) oder ein einzelner
    # grossflaechiger Glanzfleck koennen ebenfalls als eigenes Cluster
    # gewinnen - das ist ohne weiteren Bedienerhinweis nicht auflösbar und
    # bleibt ein dokumentierter Vorschlagsfehler, keine automatische Uebernahme.
    candidates = [c for c in _cluster_by_vertical_center(best_blobs, height) if len(c) >= 2]
    if not candidates:
        return None
    boxes = max(candidates, key=lambda boxes: sum(w * h for _, _, w, h in boxes))
    xs0 = min(x for x, _, _, _ in boxes)
    ys0 = min(y for _, y, _, _ in boxes)
    xs1 = max(x + w for x, _, w, _ in boxes)
    ys1 = max(y + h for _, y, _, h in boxes)
    ...
    if box_h <= 0:
        return None
    area_fraction = (box_w * box_h) / crop_area
